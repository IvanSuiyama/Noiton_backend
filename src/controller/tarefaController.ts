...
export const createTarefa = async (req: Request, res: Response) => {
  const { id_categoria, id_workspace, titulo, data_inicio, data_fim, conteudo, status } = req.body;
  ...
  try {
    await cadastrarTarefa(connection, { id_categoria, id_workspace, titulo, data_inicio, data_fim, conteudo, status });
...
export const updateTarefa = async (req: Request, res: Response) => {
  ...
  try {
    await editarTarefa(connection, parseInt(id, 10), { id_categoria, id_workspace, titulo, data_inicio, data_fim, conteudo, status });
